...
function render(el) {
  ...
  el.innerHTML = `<div class="workspace">${head('Comments', ref ? `Scoped to ${ref}` : 'All comments across the workspace.')}
    ...
    <div class="panel"><div class="thread">${items.length ? items.map(c => `<div class="msg ${c.author === s.profile.email ? 'mine' : ''}"><span class="who">${esc(c.author)} · ${esc(String(c.ts).slice(0, 16).replace('T', ' '))} ${c.referenceId ? '· ' + esc(c.referenceId) : ''}</span>${esc(c.body)}</div>`).join('') : '<p class="meta">No comments yet.</p>'}</div>
  ...
  el.querySelector('#comment-form').onsubmit = async e => {
    ...
    State.patch({ comments: [...s.comments, { id: crypto.randomUUID(), referenceId, body: d.body, author: s.profile.email, ts: new Date().toISOString() }] }, { module: 'comments', action: 'comment:add', ref: referenceId });
