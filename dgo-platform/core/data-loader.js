...
export function parseFetchAll(response){const data=assertEnvelope(response,'fetchAll'),patch={},counts={},warnings=[];for(const [target,spec] of Object.entries(specs)){const rows=collection(data,...spec.aliases);patch[target]=clean(rows).map(spec.map);counts[target]=patch[target].length;if(rows.some(x=>!x||typeof x!=='object'))warnings.push(`${target}: non-object rows skipped`)}return{patch,counts,warnings,meta:responseMeta(response)}}
